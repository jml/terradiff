...
BASE_IMAGE_NAME := quay.io/jml0/terradiff-base

# This must match the output image generated by Stack
STACK_OUTPUT_IMAGE_NAME := quay.io/jml0/terradiff-terradiff

# The image name we actually want to generate. This is what we push to an
# image registry.
OUTPUT_IMAGE_NAME := quay.io/jml0/terradiff

UPTODATE_FILES := $(UPTODATE) terradiff-base/$(UPTODATE)
IMAGE_NAMES := $(BASE_IMAGE_NAME) $(STACK_OUTPUT_IMAGE_NAME) $(OUTPUT_IMAGE_NAME)

# Build the base image
terradiff-base/$(UPTODATE): terradiff-base/Dockerfile
	$(SUDO) docker build -t $(BASE_IMAGE_NAME) terradiff-base/
	touch $@
...
$(UPTODATE): terradiff-base/$(UPTODATE) build
